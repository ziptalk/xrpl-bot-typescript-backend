            ...
            if (buyResult && sellResult) {
                ...
                if (buyResult && !sellResult) {
                    const sellLossTx = {
                        TransactionType: 'OfferCreate',
                        Account: wallet.address,
                        TakerGets: { currency: asset2.currency, value: (100 * sellPrice * (1 - maxLoss)).toString() },
                        TakerPays: { currency: asset1.currency }
                    }
                    await submitTransaction(wallet, sellLossTx)
                    logTransaction('Sell (Loss)', asset2.currency, sellPrice * (1 - maxLoss))
                }
                if (!buyResult && sellResult) {
                    const buyLossTx = {
                        TransactionType: 'OfferCreate',
                        Account: wallet.address,
                        TakerGets: { currency: asset1.currency },
                        TakerPays: { currency: asset2.currency, value: (100 * buyPrice * (1 + maxLoss)).toString() }
                    }
                    await submitTransaction(wallet, buyLossTx)
                    logTransaction('Buy (Loss)', asset1.currency, buyPrice * (1 + maxLoss))
                }
            }
        } else {
...
async function arbitrage() {
    ...
    try {
        if (useAMM) {
            ...
            // AMM에 유동성 추가
            await depositToAMM(ammResult.Account, assets[0], { currency: 'XRP', value: '1000' }) // Example values

            // AMM을 통한 거래 처리 로직 추가
            await checkArbitrage(assets[0], assets[1])
        } else {
